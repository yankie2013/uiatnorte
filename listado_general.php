...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        (new ExpedienteAccessService($pdo))->change($id, $action, (int)($_POST['usuario_id'] ?? 0), trim((string)($_POST['motivo'] ?? '')));
        $query = $_GET;
        unset($query['id']);
        $query['ok'] = '1';
...
if ($id) {
    ...
    if ($a && !$a['eliminado_en']) {
        ...
        echo '</select></label><label>Motivo de la asignación o cambio<input name="motivo" required maxlength="2000"></label><button>Guardar responsable</button></form>';
...
$from = "FROM accidentes a LEFT JOIN usuarios u ON u.id=a.responsable_id WHERE $where";
...
$offset=($page-1)*50;
$s=$pdo->prepare("SELECT a.id,a.registro_sidpol,a.lugar,a.estado,u.nombre responsable,u.grado,
...
echo '<section><p>'.$total.' registros · Todos los responsables, incluidos los expedientes sin asignar.</p><div class="table-scroll"><table><thead><tr><th>SIDPOL</th><th>Lugar del accidente</th><th>A cargo de</th><th>Estado</th><th>Tipo de accidente</th><th>Detalle</th></tr></thead><tbody>';
foreach ($rows as $row) {
    $href='listado_general.php?'.http_build_query($query+['id'=>(int)$row['id']]);
    echo '<tr><td>'.$h($row['registro_sidpol']?:'Sin SIDPOL').'</td><td>'.$h($row['lugar']).'</td><td>'.$h(trim(($row['grado']??'').' '.($row['responsable']??''))?:'Sin asignar').'</td><td>'.$h($row['estado']).'</td><td>'.$h($row['modalidad']?:'Sin registrar').'</td><td><a href="'.$h($href).'">Ver resumen y gestionar</a></td></tr>';
}
if (!$rows) echo '<tr><td colspan="6">No hay registros con estos filtros.</td></tr>';
