...
$error = '';
$success = '';
$data = $service->personaEntidadDefault(['entidad_id' => $presetEntidadId > 0 ? $presetEntidadId : '']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $service->createPersonaEntidad($data);
        $success = 'Persona vinculada registrada correctamente.';
        $data = $service->personaEntidadDefault(['entidad_id' => $presetEntidadId > 0 ? $presetEntidadId : '']);
    } catch (Throwable $e) {
        $error = $e->getMessage();
    }
...
<head>
<meta charset="utf-8">
<title>Nueva persona de entidad</title>
...
<div class="wrap">
  <div class="toolbar">
    <h1 style="margin:0;">Nueva persona vinculada</h1>
    <div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn" type="button" onclick="closeModalMaybe()">Cerrar</button><button class="btn primary" type="submit" form="frmPersonaEntidad">Guardar</button></div>
  </div>
  <?php if ($error !== ''): ?><div class="err"><?= h($error) ?></div><?php endif; ?>
  <?php if ($success !== ''): ?><div class="ok"><?= h($success) ?></div><?php endif; ?>
  <form method="post" class="card" id="frmPersonaEntidad">
    <div class="grid">
      <div class="c12 field"><label class="label">Entidad*</label><select name="entidad_id" required><option value="">Selecciona</option><?php foreach ($entidades as $entidad): ?><option value="<?= (int) $entidad['id'] ?>" <?= (string) $data['entidad_id'] === (string) $entidad['id'] ? 'selected' : '' ?>><?= h((string) $entidad['nombre']) ?></option><?php endforeach; ?></select></div>
      ...
      <div class="c4 field"><label class="label">Apellido materno</label><input type="text" name="apellido_materno" value="<?= h((string) $data['apellido_materno']) ?>"></div>
      ...
      <div class="c6 field"><label class="label">Direccion</label><input type="text" name="direccion" value="<?= h((string) $data['direccion']) ?>"></div>
      <div class="c6 field"><label class="label">Pagina web</label><input type="text" name="pagina_web" value="<?= h((string) $data['pagina_web']) ?>"></div>
      <div class="c12 field"><label class="label">Observacion</label><input type="text" name="observacion" value="<?= h((string) $data['observacion']) ?>"></div>
    </div>
