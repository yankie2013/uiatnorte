...
<div class="wrap">
  <div class="toolbar">
    <div>
      <h1 style="margin:0 0 6px">Familiar de fallecido <span class="badge">Nuevo</span></h1>
      <div class="small">Accidente ID: <?= (int) $accidenteId ?></div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a class="btn" href="familiar_fallecido_listar.php?accidente_id=<?= (int) $accidenteId ?>">Volver</a>
      <button class="btn primary" type="submit" form="frmFamiliar">Guardar registro</button>
    </div>
  </div>
  ...
  <form method="post" class="card" id="frmFamiliar" autocomplete="off">
    ...
    <div class="grid">
      ...
      <div class="c12 field">
        ...
        <div class="search-row">
          ...
          <button type="button" class="btn" id="btnNuevo">+</button>
    ...
    <div class="actions">
      <a class="btn" href="familiar_fallecido_listar.php?accidente_id=<?= (int) $accidenteId ?>">Cancelar</a>
...
(function(){
  ...
  async function buscarPorDocumento() {
    ...
    const doc = (numDoc.value || '').trim();
    info.textContent = 'Consultando en BD...';
    try {
      const r = await fetch('familiar_fallecido_nuevo.php?ajax=buscar_doc&tipo=' + encodeURIComponent(tipo) + '&doc=' + encodeURIComponent(doc), {cache:'no-store'});
  ...
  window.addEventListener('message', async function(ev){
    ...
    if (d.type === 'persona_creada' && d.id) {
      ...
      frame.src = 'about:blank';
      try {
        const r = await fetch('familiar_fallecido_nuevo.php?ajax=buscar_id&id=' + encodeURIComponent(d.id), {cache:'no-store'});
        const j = await r.json();
