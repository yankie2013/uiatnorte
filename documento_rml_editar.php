...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $row = array_merge($row, [
            ...
            'incapacidad_medico' => $_POST['incapacidad_medico'] ?? ($row['incapacidad_medico'] ?? ''),
            'atencion_facultativo' => $_POST['atencion_facultativo'] ?? ($row['atencion_facultativo'] ?? ''),
            'observaciones' => $_POST['observaciones'] ?? ($row['observaciones'] ?? ''),
        ]);
    }
}
?>
...
<body><div class="wrap">
  ...
  <form method="post" class="card" autocomplete="off">
    ...
    <div class="grid">
      ...
      <div class="col-4"><label>Fecha</label><input type="date" name="fecha" value="<?= h($row['fecha'] ?? '') ?>" required></div>
      <div class="col-4"><label>Incapacidad m&eacute;dico</label><input name="incapacidad_medico" value="<?= h($row['incapacidad_medico'] ?: '') ?>" placeholder='Ej. 3 o "No requiere"'></div>
      <div class="col-4"><label>Atenci&oacute;n facultativo</label><input name="atencion_facultativo" value="<?= h($row['atencion_facultativo'] ?: '') ?>" placeholder='Ej. 1 o "No requiere"'></div>
