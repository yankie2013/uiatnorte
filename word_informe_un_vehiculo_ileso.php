...
/* ===========================================================
   WORD: INFORME UN VEHICULO ILESO
   Entrada: ?accidente_id=ID  (acepta ?id)
   Opcional: ?vehiculo_inv_id=ID para limitar a una unidad.
   Si existe /plantillas/word_informe_un_vehiculo_ileso.docx,
...
function build_template_markers(PDO $pdo, int $accidenteId, array $accidente, array $vehicles): array
{
    $markers = [];
    foreach ([
        'titulo_informe' => 'INFORME POLICIAL - UN VEHICULO ILESO',
...
$filename = uiat_vehicle_report_filename('UT1', 'Ileso', $mainVehicle, $mainDriver, 'INFORME_UN_VEHICULO_ILESO_' . $infpol);
$templatePath = __DIR__ . '/plantillas/word_informe_un_vehiculo_ileso.docx';
...
$section->addText('INFORME POLICIAL - UN VEHICULO ILESO', ['bold' => true, 'size' => 16, 'color' => '9A6A00'], ['alignment' => 'center']);
...
foreach ($vehicles as $vehicle) {
    ...
    add_heading($section, '2.' . $vehicleNumber . ' Vehiculo involucrado: ' . $vehicleTitle);
    add_pairs($section, [
