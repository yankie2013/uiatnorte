...
<style>
:root{--page:#f6f8fc;--card:#fff;--text:#0f172a;--muted:#64748b;--border:#d7deea;--primary:#2563eb;--danger:#b91c1c;--soft:#eef4ff}
@media (prefers-color-scheme: dark){:root{--page:#0b1220;--card:#0f172a;--text:#e5e7eb;--muted:#94a3b8;--border:#23314d;--primary:#60a5fa;--danger:#fecaca;--soft:#101a2f}}
...
.name-search{border:1px dashed var(--border);border-radius:12px;padding:12px;background:var(--soft);display:flex;flex-direction:column;gap:10px}.name-search-head{display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap}.name-search-title{font-weight:800}.name-results{display:grid;gap:8px}.person-result{width:100%;display:grid;grid-template-columns:minmax(92px,120px) 1fr auto;gap:10px;align-items:center;text-align:left;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--text);padding:10px;cursor:pointer}.person-result:hover{border-color:var(--primary)}.person-dni{font-weight:900;color:var(--primary);font-size:15px}.person-name{font-weight:750}.person-meta{color:var(--muted);font-size:12px}.person-action{font-size:12px;color:var(--muted);white-space:nowrap}@media(max-width:680px){.person-result{grid-template-columns:1fr}.person-action{white-space:normal}}
</style>
</head>
<body>
<div class="wrap">
  ...
  <form method="post" class="card" id="frmPersona" autocomplete="off">
    ...
    <div class="grid">
      ...
      <div class="c12 name-search">
        <div class="name-search-head">
          <div>
            <div class="name-search-title">Buscar DNI por nombres y apellidos</div>
            <div class="small">Ingresa nombres y al menos un apellido. Selecciona un resultado para copiar el DNI y cargar RENIEC.</div>
          </div>
          <button class="btn" type="button" id="btnBuscarNombre">Buscar por nombre</button>
        </div>
        <div class="grid">
          <div class="c4 field"><label class="label">Nombres</label><input type="text" id="buscar_nombres" autocomplete="off"></div>
          <div class="c3 field"><label class="label">Apellido paterno</label><input type="text" id="buscar_paterno" autocomplete="off"></div>
          <div class="c3 field"><label class="label">Apellido materno</label><input type="text" id="buscar_materno" autocomplete="off"></div>
          <div class="c3 field"><label class="label">Edad minima</label><input type="number" id="buscar_edad_min" min="0" max="120"></div>
          <div class="c3 field"><label class="label">Edad maxima</label><input type="number" id="buscar_edad_max" min="0" max="120"></div>
        </div>
        <div class="small" id="nombre_hint">Sin busqueda realizada.</div>
        <div class="name-results" id="nombre_resultados"></div>
      </div>
      ...
      <div class="c12 field"><label class="label">Notas</label><textarea name="notas" id="notas"><?= h((string) $data['notas']) ?></textarea></div>
    </div>
...
function cleanSpaces(value){return String(value??'').trim().replace(/\s+/g,' ');}
function titleText(value){const text=cleanSpaces(value).toLocaleLowerCase('es-PE');return text.replace(/(^|[\s.'-])([\p{L}])/gu,(m,sep,letter)=>sep+letter.toLocaleUpperCase('es-PE'));}
function upperText(value){return cleanSpaces(value).toLocaleUpperCase('es-PE');}
function addressText(value){let text=titleText(value);const rules=[[/\bMza?\.?\b/giu,'MZ'],[/\bManzana\b/giu,'MZ'],[/\bLt\.?\b/giu,'Lote'],[/\bLote\b/giu,'Lote'],[/\bNro\.?\b/giu,'Nro.'],[/\bNumero\b/giu,'Nro.'],[/\bNúmero\b/giu,'Nro.'],[/\bN[°º]/giu,'Nro.'],[/\bAv\.?\b/giu,'Av.'],[/\bAvenida\b/giu,'Av.'],[/\bJr\.?\b/giu,'Jr.'],[/\bJiron\b/giu,'Jr.'],[/\bJirón\b/giu,'Jr.'],[/\bPsje\.?\b/giu,'Psje.'],[/\bPasaje\b/giu,'Psje.'],[/\bDpto\.?\b/giu,'Dpto.'],[/\bDepartamento\b/giu,'Dpto.'],[/\bInt\.?\b/giu,'Int.'],[/\bInterior\b/giu,'Int.']];rules.forEach(([pattern,replacement])=>{text=text.replace(pattern,replacement);});return cleanSpaces(text);}
function appendNote(value){const note=cleanSpaces(value);if(!note){return;}const field=document.getElementById('notas');if(!field){return;}if(field.value.includes(note)){return;}field.value=field.value.trim()?field.value.trim()+"\n"+note:note;}
async function buscarRENIEC(){const tipo=document.getElementById('tipo_doc').value;let doc=(document.getElementById('num_doc').value||'').trim().replace(/\D/g,'');if(tipo!=='DNI'){alert('Selecciona DNI');return;}if(doc.length!==8){alert('Escribe 8 digitos de DNI');return;}const btn=document.getElementById('btnBuscarReniec');const hint=document.getElementById('dni_hint');btn.disabled=true;hint.textContent='Consultando RENIEC...';try{const fd=new FormData();fd.append('dni',doc);fd.append('guardar','0');const r=await fetch('buscar_dni.php',{method:'POST',body:fd});const j=await r.json();if(!j.ok)throw new Error(j.msg||'No se pudo consultar');const d=j.data||{};setFieldValue('tipo_doc',d.tipo_doc||'DNI');setFieldValue('num_doc',d.num_doc||doc);setFieldValue('apellido_paterno',upperText(d.apellido_paterno||''));setFieldValue('apellido_materno',upperText(d.apellido_materno||''));setFieldValue('nombres',titleText(d.nombres||''));setFieldValue('sexo',d.sexo||'');setFieldValue('fecha_nacimiento',d.fecha_nacimiento||'');setFieldValue('estado_civil',titleText(d.estado_civil||''));setFieldValue('nacionalidad',titleText(d.nacionalidad||(tipo==='DNI'?'PERUANA':'')));setFieldValue('departamento_nac',titleText(d.departamento_nac||''));setFieldValue('provincia_nac',titleText(d.provincia_nac||''));setFieldValue('distrito_nac',titleText(d.distrito_nac||''));setFieldValue('domicilio',addressText(d.domicilio||''));setFieldValue('grado_instruccion',titleText(d.grado_instruccion||''));setFieldValue('nombre_padre',titleText(d.nombre_padre||''));setFieldValue('nombre_madre',titleText(d.nombre_madre||''));setFieldValue('domicilio_departamento',titleText(d.domicilio_departamento||''));setFieldValue('domicilio_provincia',titleText(d.domicilio_provincia||''));setFieldValue('domicilio_distrito',titleText(d.domicilio_distrito||''));setFieldValue('foto_path',d.foto_path||'');setFieldValue('api_fuente',d.api_fuente||'');setFieldValue('api_ref',d.api_ref||'');appendNote(d.notas||'');const img=document.getElementById('foto_previa');if(d.foto_path){img.src=d.foto_path;img.style.display='block';}else{img.removeAttribute('src');img.style.display='none';}document.getElementById('fecha_nacimiento').dispatchEvent(new Event('change'));hint.textContent='Datos cargados desde RENIEC. Revisa y guarda manualmente.';}catch(e){hint.textContent=e.message||'Error consultando RENIEC.';}finally{btn.disabled=false;}}
async function copyDniToClipboard(dni){try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(dni);return true;}}catch(e){}try{const tmp=document.createElement('textarea');tmp.value=dni;tmp.setAttribute('readonly','');tmp.style.position='fixed';tmp.style.left='-999px';document.body.appendChild(tmp);tmp.select();const ok=document.execCommand('copy');tmp.remove();return ok;}catch(e){return false;}}
function makeTextNode(tag,className,text){const el=document.createElement(tag);if(className){el.className=className;}el.textContent=text||'';return el;}
function renderNombreResultados(items){const target=document.getElementById('nombre_resultados');const hint=document.getElementById('nombre_hint');target.textContent='';if(!items.length){hint.textContent='No se encontraron resultados.';return;}hint.textContent=items.length+' resultado(s). Selecciona un DNI para cargar la ficha.';items.forEach(item=>{const btn=document.createElement('button');btn.type='button';btn.className='person-result';btn.dataset.dni=item.dni||'';const dni=makeTextNode('div','person-dni',item.dni||'');const body=document.createElement('div');body.appendChild(makeTextNode('div','person-name',item.nombre_completo||[item.paterno,item.materno,item.nombres].filter(Boolean).join(' ')));body.appendChild(makeTextNode('div','person-meta',item.edad?('Edad: '+item.edad):'Edad no disponible'));btn.appendChild(dni);btn.appendChild(body);btn.appendChild(makeTextNode('div','person-action','Copiar y buscar'));btn.addEventListener('click',async()=>{const selectedDni=(item.dni||'').replace(/\D/g,'');if(selectedDni.length!==8){return;}await copyDniToClipboard(selectedDni);setFieldValue('tipo_doc','DNI');setFieldValue('num_doc',selectedDni);document.getElementById('dni_hint').textContent='DNI copiado. Consultando RENIEC...';buscarRENIEC();});target.appendChild(btn);});}
async function buscarPorNombres(){const btn=document.getElementById('btnBuscarNombre');const hint=document.getElementById('nombre_hint');const target=document.getElementById('nombre_resultados');const nombres=cleanSpaces(document.getElementById('buscar_nombres')?.value||'');const paterno=cleanSpaces(document.getElementById('buscar_paterno')?.value||'');const materno=cleanSpaces(document.getElementById('buscar_materno')?.value||'');const edadMin=cleanSpaces(document.getElementById('buscar_edad_min')?.value||'');const edadMax=cleanSpaces(document.getElementById('buscar_edad_max')?.value||'');if(nombres.length<2){hint.textContent='Escribe al menos 2 caracteres en nombres.';return;}if(!paterno&&!materno){hint.textContent='Escribe apellido paterno o materno.';return;}btn.disabled=true;target.textContent='';hint.textContent='Buscando coincidencias...';try{const fd=new FormData();fd.append('nombres',nombres);fd.append('paterno',paterno);fd.append('materno',materno);fd.append('edadMin',edadMin);fd.append('edadMax',edadMax);const r=await fetch('buscar_personas_nombre.php',{method:'POST',body:fd});const j=await r.json();if(!j.ok)throw new Error(j.msg||'No se pudo buscar por nombre');renderNombreResultados((j.data&&j.data.resultados)||[]);}catch(e){hint.textContent=e.message||'Error buscando por nombres.';}finally{btn.disabled=false;}}
document.getElementById('btnBuscarReniec')?.addEventListener('click',buscarRENIEC);
document.getElementById('btnBuscarNombre')?.addEventListener('click',buscarPorNombres);
document.getElementById('num_doc')?.addEventListener('keydown',function(ev){if(ev.key==='Enter'){ev.preventDefault();buscarRENIEC();}});
['buscar_nombres','buscar_paterno','buscar_materno','buscar_edad_min','buscar_edad_max'].forEach(id=>{document.getElementById(id)?.addEventListener('keydown',function(ev){if(ev.key==='Enter'){ev.preventDefault();buscarPorNombres();}});});
</script>
</body>
</html>
