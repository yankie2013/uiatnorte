...
function vehiculo_label(array $row): string {
    $parts = [];
    if (!empty($row['placa'])) $parts[] = $row['placa'];
    if (!empty($row['color'])) $parts[] = $row['color'];
    if (!empty($row['anio'])) $parts[] = '(' . $row['anio'] . ')';
    return $parts ? implode(' ', $parts) : ('ID ' . (int)($row['vehiculo_id'] ?? 0));
}
...
if ($returnTo === '') {
    $returnTo = 'involucrados_vehiculos_editar.php?id=' . (int)$row['invol_id'];
}
...
<title>Eliminar Documento de Vehiculo</title>
<style>
...
<body>
<div class="wrap">
  <h1 style="margin-top:0">Eliminar documento de vehiculo #<?= (int)$id ?></h1>
  <?php if($error): ?><div class="err">Error: <?= h($error) ?></div><?php endif; ?>
  <p>Se eliminara el documento asociado a este vehiculo de forma permanente.</p>

  <div class="meta">
    <div class="row"><div class="lbl">Vehiculo</div><div class="val"><?= h(vehiculo_label($row)) ?></div></div>
    <div class="row"><div class="lbl">SOAT</div><div class="val"><?= h($row['numero_soat'] ?? '-') ?></div></div>
    <div class="row"><div class="lbl">Revision tecnica</div><div class="val"><?= h($row['numero_revision'] ?? '-') ?></div></div>
    <div class="row"><div class="lbl">Peritaje</div><div class="val"><?= h($row['numero_peritaje'] ?? '-') ?></div></div>
  </div>
