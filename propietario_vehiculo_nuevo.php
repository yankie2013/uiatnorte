...
if (isset($_GET['ajax'])) {
    ...
    try {
        ...
        if ($op === 'buscar_personas') {
            echo json_encode(['ok' => true, 'personas' => $service->buscarPersonas((string) ($_GET['q'] ?? ''))], JSON_UNESCAPED_UNICODE);
            exit;
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $service->create($data);
        header('Location: propietario_vehiculo_listar.php?accidente_id=' . $accidenteId . '&ok=created');
        exit;
    } catch (Throwable $e) {
        $error = $e->getMessage();
    }
...
<head>
<meta charset="utf-8">
<title>Nuevo propietario de vehiculo</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
:root{--page:#f6f8fc;--card:#fff;--text:#0f172a;--muted:#64748b;--border:#d7deea;--primary:#ca8a04;--danger:#b91c1c}
@media (prefers-color-scheme: dark){:root{--page:#0b1220;--card:#0f172a;--text:#e5e7eb;--muted:#94a3b8;--border:#23314d;--primary:#facc15;--danger:#fecaca}}
...
<div class="wrap">
  <div class="toolbar">
    <div>
      <h1 style="margin:0 0 6px">Propietario de vehiculo <span class="badge">Nuevo</span></h1>
      <div class="small">Accidente ID: <?= (int) $accidenteId ?></div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a class="btn" href="propietario_vehiculo_listar.php?accidente_id=<?= (int) $accidenteId ?>">Volver</a>
      <button class="btn primary" type="submit" form="frmProp">Guardar registro</button>
    </div>
  </div>
  ...
  <form method="post" class="card" id="frmProp" autocomplete="off">
    ...
    <div class="grid">
      <div class="c12 field">
        <label class="label" for="vehiculo_inv_id">Vehiculo del accidente*</label>
        <select name="vehiculo_inv_id" id="vehiculo_inv_id" required>
          <option value="">Selecciona</option>
          <?php foreach ($ctx['vehiculos'] as $vehiculo): ?>
            <?php $label = trim((string) (($vehiculo['orden_participacion'] ?? '') !== '' ? $vehiculo['orden_participacion'] . ' - ' : '') . ($vehiculo['placa'] ?? 'SIN PLACA')); ?>
            <option value="<?= (int) $vehiculo['inv_id'] ?>" <?= (string) $data['vehiculo_inv_id'] === (string) $vehiculo['inv_id'] ? 'selected' : '' ?>><?= h($label) ?></option>
          <?php endforeach; ?>
        </select>
      </div>

      <div class="c12 field">
        <label class="label" for="tipo_propietario">Tipo de propietario*</label>
        <select name="tipo_propietario" id="tipo_propietario">
          <?php foreach ($ctx['tipos'] as $tipo): ?>
            <option value="<?= h($tipo) ?>" <?= $data['tipo_propietario'] === $tipo ? 'selected' : '' ?>><?= h($tipo) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <div class="c12 segment" id="bloque_natural">
        <div class="field">
          <label class="label">Propietario natural*</label>
          <div class="search-row">
            <input type="text" id="dni_nat" placeholder="DNI, nombre o apellidos del propietario">
            <button type="button" class="btn" id="btnBuscarNat">Buscar</button>
            <button type="button" class="btn" id="btnNuevoNat">+</button>
            <button type="button" class="btn" id="btnLimpiarNat">Limpiar</button>
          </div>
          <div class="suggest-list" id="sug_nat"></div>
          <div class="small" id="info_nat">-</div>
        </div>
      ...
      <div class="c12 segment" id="bloque_juridica" style="display:none;">
        <div class="grid">
          <div class="c6 field"><label class="label">RUC*</label><input type="text" name="ruc" id="ruc" maxlength="11" value="<?= h((string) $data['ruc']) ?>"></div>
          <div class="c6 field"><label class="label">Rol legal</label><input type="text" name="rol_legal" value="<?= h((string) $data['rol_legal']) ?>"></div>
          <div class="c12 field"><label class="label">Razon social*</label><input type="text" name="razon_social" value="<?= h((string) $data['razon_social']) ?>"></div>
          <div class="c12 field"><label class="label">Domicilio fiscal</label><input type="text" name="domicilio_fiscal" value="<?= h((string) $data['domicilio_fiscal']) ?>"></div>
        </div>
        <div class="field" style="margin-top:12px;">
          <label class="label">Representante legal*</label>
          <div class="search-row">
            <input type="text" id="dni_rep" placeholder="DNI, nombre o apellidos del representante">
            <button type="button" class="btn" id="btnBuscarRep">Buscar</button>
            <button type="button" class="btn" id="btnNuevoRep">+</button>
            <button type="button" class="btn" id="btnLimpiarRep">Limpiar</button>
          </div>
          <div class="suggest-list" id="sug_rep"></div>
          <div class="small" id="info_rep">-</div>
        </div>
        <div class="grid" style="margin-top:12px;">
    ...
    <div class="actions">
      <a class="btn" href="propietario_vehiculo_listar.php?accidente_id=<?= (int) $accidenteId ?>">Cancelar</a>
...
<div class="modal-backdrop" id="personaModal">
  <div class="modal">
    <header>
      <h3 id="personaModalTitle">Persona</h3>
...
<script>
(function(){
  const tipoSel = document.getElementById('tipo_propietario');
  const blqNat = document.getElementById('bloque_natural');
  const blqJur = document.getElementById('bloque_juridica');
  const modal = document.getElementById('personaModal');
  const frame = document.getElementById('personaFrame');
  const title = document.getElementById('personaModalTitle');

  function toggleTipo() {
    const isNatural = tipoSel.value === 'NATURAL';
    blqNat.style.display = isNatural ? 'block' : 'none';
    blqJur.style.display = isNatural ? 'none' : 'block';
  }
  tipoSel.addEventListener('change', toggleTipo);
  toggleTipo();

  function renderPersona(prefix, persona, hiddenId) {
    document.getElementById(hiddenId).value = persona.id || '';
    document.getElementById(prefix + '_nombre').value = [persona.apellido_paterno || '', persona.apellido_materno || '', persona.nombres || ''].join(' ').trim();
    document.getElementById(prefix + '_dom').value = persona.domicilio || '';
    document.getElementById(prefix + '_cel').value = persona.celular || '';
    document.getElementById(prefix + '_email').value = persona.email || '';
    document.getElementById('info_' + prefix).innerHTML = '<b>' + document.getElementById(prefix + '_nombre').value + '</b> (id: ' + persona.id + ')';
    if (persona.num_doc) {
      document.getElementById('dni_' + prefix).value = persona.num_doc;
    }
  }
  ...
  function attachSearch(prefix, hiddenId) {
    const input = document.getElementById('dni_' + prefix);
    const suggest = document.getElementById('sug_' + prefix);
    const btnBuscar = document.getElementById('btnBuscar' + (prefix === 'nat' ? 'Nat' : 'Rep'));
    const btnNuevo = document.getElementById('btnNuevo' + (prefix === 'nat' ? 'Nat' : 'Rep'));
    const btnLimpiar = document.getElementById('btnLimpiar' + (prefix === 'nat' ? 'Nat' : 'Rep'));
    ...
    input.addEventListener('input', async function(){
      ...
      try {
        const j = await fetchJson('propietario_vehiculo_nuevo.php?ajax=buscar_personas&q=' + encodeURIComponent(q));
        if (!j.ok || !j.personas || !j.personas.length) {
          suggest.innerHTML = '<div class="suggest-item">Sin coincidencias</div>';
          suggest.style.display = 'block';
          return;
        }
        suggest.innerHTML = j.personas.map(function(persona){
          const nombre = [persona.apellido_paterno || '', persona.apellido_materno || '', persona.nombres || ''].join(' ').trim();
          return '<div class="suggest-item" data-id="' + persona.id + '">' + nombre + ' - ' + (persona.num_doc || '') + '</div>';
        }).join('');
        suggest.style.display = 'block';
      } catch (e) {
        suggest.style.display = 'none';
    ...
    suggest.addEventListener('click', async function(ev){
      const item = ev.target.closest('.suggest-item');
      if (!item || !item.dataset.id) return;
      const j = await fetchJson('propietario_vehiculo_nuevo.php?ajax=buscar_id&id=' + encodeURIComponent(item.dataset.id));
      if (j.ok && j.persona) renderPersona(prefix, j.persona, hiddenId);
      suggest.style.display = 'none';
      suggest.innerHTML = '';
    });
    ...
    btnBuscar.addEventListener('click', async function(){
      ...
      const j = await fetchJson('propietario_vehiculo_nuevo.php?ajax=buscar_dni&dni=' + encodeURIComponent(dni));
      if (!j.ok) {
        document.getElementById('info_' + prefix).textContent = j.msg || 'No encontrado.';
        return;
      }
      renderPersona(prefix, j.persona, hiddenId);
    });

    btnNuevo.addEventListener('click', function(){
      title.textContent = 'Nueva persona';
      frame.src = 'persona_nuevo.php' + (((input.value || '').trim() !== '') ? ('?dni=' + encodeURIComponent(input.value.trim())) : '');
      modal.style.display = 'flex';
      modal.dataset.target = prefix;
    });

    btnLimpiar.addEventListener('click', function(){
      document.getElementById(hiddenId).value = '';
      document.getElementById(prefix + '_nombre').value = '';
      document.getElementById(prefix + '_dom').value = '';
      document.getElementById(prefix + '_cel').value = '';
      document.getElementById(prefix + '_email').value = '';
      document.getElementById('info_' + prefix).textContent = '-';
      input.value = '';
    });
  }

  attachSearch('nat', 'propietario_persona_id');
  attachSearch('rep', 'representante_persona_id');
  ...
  window.addEventListener('message', async function(ev){
    ...
    if (d.type === 'persona_creada' && d.id) {
      const target = modal.dataset.target || 'nat';
      const hiddenId = target === 'nat' ? 'propietario_persona_id' : 'representante_persona_id';
      const j = await fetchJson('propietario_vehiculo_nuevo.php?ajax=buscar_id&id=' + encodeURIComponent(d.id));
      if (j.ok && j.persona) renderPersona(target, j.persona, hiddenId);
      modal.style.display = 'none';
      frame.src = 'about:blank';
    }
  });
