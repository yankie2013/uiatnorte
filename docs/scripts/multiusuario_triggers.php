    ...
    foreach(['INSERT'=>'NEW','UPDATE'=>'NEW','DELETE'=>'OLD'] as $event=>$row) {
        ...
        if($table==='accidentes') {
            ...
            if($event==='INSERT') {
                ...
                $guard .= " OR (COALESCE(@rbac_assignment,0)=1 AND OLD.eliminado_en IS NULL AND EXISTS(SELECT 1 FROM expediente_transferencias t JOIN usuarios u ON u.id=t.destino_id WHERE t.accidente_id=OLD.id AND t.origen_id=OLD.responsable_id AND t.destino_id=@actor_id AND t.estado='pendiente' AND u.activo=1 AND u.rol='jefe_emi'))";
