...
if (isset($_GET['ajax'])) {
    ...
    try {
        ...
        if ($op === 'buscar_id') {
            $persona = $service->personaPorId((int) ($_GET['id_persona'] ?? 0));
            echo json_encode(['ok' => true, 'persona' => $persona], JSON_UNESCAPED_UNICODE);
            exit;
        }
...
<head>
<meta charset="utf-8">
<title>Editar interviniente policial</title>
...
<div class="wrap">
  <div class="toolbar">
    <div><h1 style="margin:0 0 6px">Interviniente policial <span class="badge">Editar</span></h1><div class="small">Registro #<?= (int) $id ?> - Accidente ID: <?= (int) $accidenteId ?></div></div>
    <div style="display:flex;gap:10px;flex-wrap:wrap"><a class="btn" href="<?= h($returnTo) ?>">Volver</a><a class="btn" href="policial_interviniente_leer.php?id=<?= (int) $id ?>&return_to=<?= urlencode($returnTo) ?>">Ver detalle</a><button class="btn primary" type="submit" form="frmPolicia">Guardar cambios</button></div>
  </div>
  ...
  <form method="post" class="card" id="frmPolicia" autocomplete="off">
    ...
    <div class="actions"><a class="btn" href="<?= h($returnTo) ?>">Cancelar</a><button class="btn primary" type="submit">Guardar</button></div>
  </form>
</div>
...
<script>
(function(){const dni=document.getElementById('dni');const info=document.getElementById('persona_info');const personaId=document.getElementById('persona_id');const nombre=document.getElementById('persona_nombre');const domicilio=document.getElementById('persona_domicilio');const celular=document.getElementById('persona_celular');const email=document.getElementById('persona_email');const modal=document.getElementById('personaModal');const frame=document.getElementById('personaFrame');function pintar(persona){if(!persona){return;}personaId.value=persona.id||'';nombre.value=[persona.apellido_paterno||'',persona.apellido_materno||'',persona.nombres||''].join(' ').trim();domicilio.value=persona.domicilio||'';celular.value=persona.celular||'';email.value=persona.email||'';if(persona.num_doc)dni.value=persona.num_doc;info.innerHTML='Persona: <b>'+nombre.value+'</b> (id: '+persona.id+')';}async function buscarPorDni(){try{const r=await fetch('policial_interviniente_editar.php?id=<?= (int) $id ?>&ajax=buscar_dni&dni='+encodeURIComponent((dni.value||'').trim()),{cache:'no-store'});const j=await r.json();if(!j.ok){info.textContent=j.msg||'No encontrado.';return;}pintar(j.persona);}catch(e){info.textContent='Error consultando.';}}async function recargarActual(){if(!personaId.value){return;}try{const r=await fetch('policial_interviniente_editar.php?id=<?= (int) $id ?>&ajax=buscar_id&id_persona='+encodeURIComponent(personaId.value),{cache:'no-store'});const j=await r.json();if(j.ok&&j.persona)pintar(j.persona);}catch(e){}}document.getElementById('btnBuscar').addEventListener('click',buscarPorDni);document.getElementById('btnRecargar').addEventListener('click',recargarActual);document.getElementById('btnNuevo').addEventListener('click',function(){frame.src='persona_nuevo.php'+(((dni.value||'').trim()!=='')?('?dni='+encodeURIComponent(dni.value.trim())):'');modal.style.display='flex';});document.getElementById('btnModalCerrar').addEventListener('click',function(){modal.style.display='none';frame.src='about:blank';});window.addEventListener('message',async function(ev){const d=ev.data||{};if(d.type==='persona_creada'&&d.id){const r=await fetch('policial_interviniente_editar.php?id=<?= (int) $id ?>&ajax=buscar_id&id_persona='+encodeURIComponent(d.id),{cache:'no-store'});const j=await r.json();if(j.ok&&j.persona)pintar(j.persona);modal.style.display='none';frame.src='about:blank';}});})();
</script>
</body>
</html>
