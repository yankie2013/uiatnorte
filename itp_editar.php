...
$error = '';
$data = $service->defaultData($detail, (int) $detail['accidente_id']);
$backUrl = trim((string) ($_POST['back_url'] ?? $_GET['back'] ?? ('itp_ver.php?id=' . $id)));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        header('Location: itp_ver.php?id=' . $id . '&ok=updated');
        exit;
    } catch (Throwable $e) {
        $error = $e->getMessage();
    }
...
<head>
<meta charset="utf-8">
<title>Editar ITP</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
:root{--page:#f6f8fc;--card:#fff;--text:#0f172a;--muted:#64748b;--border:#d7deea;--primary:#0f766e;--danger:#b91c1c;--gold:#b68b1f}
@media (prefers-color-scheme: dark){:root{--page:#0b1220;--card:#0f172a;--text:#e5e7eb;--muted:#94a3b8;--border:#23314d;--primary:#5eead4;--danger:#fecaca;--gold:#e6c97d}}
*{box-sizing:border-box}body{margin:0;background:var(--page);color:var(--text);font:14px/1.45 Inter,system-ui,Segoe UI,Roboto,Arial,sans-serif}.wrap{max-width:1160px;margin:24px auto;padding:0 12px}.toolbar{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:14px}h1,h2{color:var(--gold);margin:0}.badge{display:inline-block;padding:3px 8px;border-radius:999px;background:rgba(15,118,110,.12);color:var(--primary);border:1px solid rgba(15,118,110,.18);font-size:11px}.btn{padding:10px 14px;border-radius:10px;border:1px solid var(--border);background:var(--card);color:var(--text);font-weight:700;text-decoration:none;cursor:pointer}.btn.primary{background:var(--primary);color:#ecfeff;border-color:transparent}.btn.ghost{background:transparent}.card{background:var(--card);border:1px solid var(--border);border-radius:16px;padding:16px;margin-bottom:14px}.fieldset{border:1px solid var(--border);border-radius:14px;padding:14px;margin-top:14px}.grid{display:grid;grid-template-columns:repeat(12,1fr);gap:12px}.c12{grid-column:span 12}.c6{grid-column:span 6}.c4{grid-column:span 4}.c3{grid-column:span 3}.field{display:flex;flex-direction:column;gap:6px}.label{font-size:12px;color:var(--gold);font-weight:700}.small{color:var(--muted);font-size:12px}.err{background:rgba(220,38,38,.12);color:var(--danger);padding:10px;border-radius:10px;margin-bottom:12px}input,select,textarea{width:100%;padding:10px 12px;border:1px solid var(--border);border-radius:12px;background:transparent;color:var(--text)}textarea{min-height:92px;resize:vertical}.actions{display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin-top:16px}.tagbox,.measurebox,.pairbox{border:1px dashed var(--border);border-radius:12px;padding:10px}.tag-items,.measure-list,.pair-list{display:flex;flex-direction:column;gap:8px;margin-top:10px}.chip,.measure-item,.pair-item{display:flex;justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;border:1px solid var(--border);border-radius:10px;background:rgba(148,163,184,.08)}.mini-row{display:flex;gap:8px;flex-wrap:wrap}.mini-row input{flex:1}.gps-row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.gps-row input{flex:1}.gps-modal{position:fixed;inset:0;display:none;z-index:60}.gps-backdrop{position:absolute;inset:0;background:rgba(15,23,42,.65)}.gps-panel{position:relative;max-width:760px;margin:50px auto 0;background:var(--card);border:1px solid var(--border);border-radius:18px;padding:14px}.gps-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px}.gps-search input{width:100%;margin-bottom:10px}.gps-map{height:380px;border:1px solid var(--border);border-radius:12px;overflow:hidden}@media(max-width:920px){.c6,.c4,.c3{grid-column:span 12}}
</style>
</head>
<body>
<div class="wrap">
  <div class="toolbar">
    <div><h1>ITP <span class="badge">Editar</span></h1><div class="small">Registro #<?= (int) $id ?></div></div>
    <div style="display:flex;gap:10px;flex-wrap:wrap"><a class="btn ghost" href="<?= h($backUrl) ?>">Volver</a><button class="btn primary" type="submit" form="frmItp">Guardar cambios</button></div>
  </div>
  <?php if ($error !== ''): ?><div class="err"><?= h($error) ?></div><?php endif; ?>
  <form method="post" id="frmItp" class="card" autocomplete="off">
    <input type="hidden" name="id" value="<?= (int) $id ?>">
    <input type="hidden" name="back_url" value="<?= h($backUrl) ?>">
    <div class="fieldset"><strong>Accidente #<?= (int) $detail['accidente_id'] ?></strong><div class="small" style="margin-top:4px;">SIDPOL: <?= h((string) ($detail['registro_sidpol'] ?? '-')) ?> - Fecha: <?= h((string) ($detail['fecha_accidente'] ?? '-')) ?> - Lugar: <?= h((string) ($detail['lugar'] ?? '-')) ?></div></div>
    <div class="fieldset"><h2 style="font-size:1rem;margin-bottom:10px;">Datos generales</h2><div class="grid"><div class="c3 field"><label class="label">Fecha ITP</label><input type="date" name="fecha_itp" value="<?= h((string) $data['fecha_itp']) ?>"></div><div class="c3 field"><label class="label">Hora ITP</label><input type="time" name="hora_itp" value="<?= h((string) $data['hora_itp']) ?>"></div><div class="c6 field"><label class="label">Forma de la via</label><input type="text" name="forma_via" value="<?= h((string) $data['forma_via']) ?>"></div><div class="c12 field"><label class="label">Punto de referencia</label><input type="text" name="punto_referencia" value="<?= h((string) $data['punto_referencia']) ?>"></div><div class="c12 field"><label class="label">Ubicacion GPS</label><div class="gps-row"><input type="text" id="ubicacion_gps" name="ubicacion_gps" value="<?= h((string) $data['ubicacion_gps']) ?>" readonly><button type="button" class="btn" id="btnGps">Marcar en mapa</button><button type="button" class="btn ghost" id="btnViewMap">Ver mapa</button><a class="btn ghost" id="btnGoogleMaps" href="#" target="_blank" rel="noopener noreferrer">Google Maps</a></div><div class="small">Se usa automaticamente el punto registrado en Datos Generales del accidente.</div></div><div class="c12 field"><label class="label">Localizacion de unidades</label><div class="pairbox" id="pb_localizacion" data-values="<?= h((string) $data['localizacion_unidades']) ?>"><div class="mini-row"><input type="text" class="p-unit" placeholder="Unidad"><input type="text" class="p-place" placeholder="Ubicacion"><button type="button" class="btn">Agregar</button></div><div class="pair-list"></div><input type="hidden" name="localizacion_unidades" value="<?= h((string) $data['localizacion_unidades']) ?>"></div></div><div class="c12 field"><label class="label">Ocurrencia policial</label><textarea name="ocurrencia_policial"><?= h((string) $data['ocurrencia_policial']) ?></textarea></div><div class="c12 field"><label class="label">Llegada al lugar</label><textarea name="llegada_lugar"><?= h((string) $data['llegada_lugar']) ?></textarea></div></div></div>
    <div class="fieldset"><h2 style="font-size:1rem;margin-bottom:10px;">Via 1</h2><div class="grid"><div class="c12 field"><label class="label">Descripcion</label><textarea name="descripcion_via1"><?= h((string) $data['descripcion_via1']) ?></textarea></div><div class="c4 field"><label class="label">Configuracion</label><input type="text" name="configuracion_via1" value="<?= h((string) $data['configuracion_via1']) ?>"></div><div class="c4 field"><label class="label">Material</label><input type="text" name="material_via1" value="<?= h((string) $data['material_via1']) ?>"></div><div class="c4 field"><label class="label">Senializacion</label><input type="text" name="senializacion_via1" value="<?= h((string) $data['senializacion_via1']) ?>"></div><div class="c4 field"><label class="label">Ordenamiento</label><input type="text" name="ordenamiento_via1" value="<?= h((string) $data['ordenamiento_via1']) ?>"></div><div class="c4 field"><label class="label">Iluminacion</label><input type="text" name="iluminacion_via1" value="<?= h((string) $data['iluminacion_via1']) ?>"></div><div class="c4 field"><label class="label">Visibilidad</label><input type="text" name="visibilidad_via1" value="<?= h((string) $data['visibilidad_via1']) ?>"></div><div class="c6 field"><label class="label">Intensidad</label><input type="text" name="intensidad_via1" value="<?= h((string) $data['intensidad_via1']) ?>"></div><div class="c6 field"><label class="label">Fluidez</label><input type="text" name="fluidez_via1" value="<?= h((string) $data['fluidez_via1']) ?>"></div><div class="c12 field"><label class="label">Medidas</label><div class="measurebox" id="mb_via1" data-values="<?= h((string) $data['medidas_via1']) ?>"><div class="mini-row"><input type="text" class="m-name" placeholder="Que mides"><input type="text" class="m-value" placeholder="Valor"><button type="button" class="btn m-add">Agregar</button></div><div class="measure-list"></div><input type="hidden" name="medidas_via1" value="<?= h((string) $data['medidas_via1']) ?>"></div></div><div class="c12 field"><label class="label">Observaciones</label><div class="tagbox" id="tl_obs_via1" data-values="<?= h((string) $data['observaciones_via1']) ?>"><div class="tag-items"></div><div class="mini-row"><input type="text" placeholder="Agregar observacion"><button type="button" class="btn">Agregar</button></div><input type="hidden" name="observaciones_via1" value="<?= h((string) $data['observaciones_via1']) ?>"></div></div></div></div>
    ...
    <div class="fieldset" id="fs_via2" <?= (int) $data['via2_flag'] === 1 ? '' : 'style="display:none;"' ?>><h2 style="font-size:1rem;margin-bottom:10px;">Via 2</h2><div class="grid"><div class="c12 field"><label class="label">Descripcion</label><textarea name="descripcion_via2"><?= h((string) $data['descripcion_via2']) ?></textarea></div><div class="c4 field"><label class="label">Configuracion</label><input type="text" name="configuracion_via2" value="<?= h((string) $data['configuracion_via2']) ?>"></div><div class="c4 field"><label class="label">Material</label><input type="text" name="material_via2" value="<?= h((string) $data['material_via2']) ?>"></div><div class="c4 field"><label class="label">Senializacion</label><input type="text" name="senializacion_via2" value="<?= h((string) $data['senializacion_via2']) ?>"></div><div class="c4 field"><label class="label">Ordenamiento</label><input type="text" name="ordenamiento_via2" value="<?= h((string) $data['ordenamiento_via2']) ?>"></div><div class="c4 field"><label class="label">Iluminacion</label><input type="text" name="iluminacion_via2" value="<?= h((string) $data['iluminacion_via2']) ?>"></div><div class="c4 field"><label class="label">Visibilidad</label><input type="text" name="visibilidad_via2" value="<?= h((string) $data['visibilidad_via2']) ?>"></div><div class="c6 field"><label class="label">Intensidad</label><input type="text" name="intensidad_via2" value="<?= h((string) $data['intensidad_via2']) ?>"></div><div class="c6 field"><label class="label">Fluidez</label><input type="text" name="fluidez_via2" value="<?= h((string) $data['fluidez_via2']) ?>"></div><div class="c12 field"><label class="label">Medidas</label><div class="measurebox" id="mb_via2" data-values="<?= h((string) $data['medidas_via2']) ?>"><div class="mini-row"><input type="text" class="m-name" placeholder="Que mides"><input type="text" class="m-value" placeholder="Valor"><button type="button" class="btn m-add">Agregar</button></div><div class="measure-list"></div><input type="hidden" name="medidas_via2" value="<?= h((string) $data['medidas_via2']) ?>"></div></div><div class="c12 field"><label class="label">Observaciones</label><div class="tagbox" id="tl_obs_via2" data-values="<?= h((string) $data['observaciones_via2']) ?>"><div class="tag-items"></div><div class="mini-row"><input type="text" placeholder="Agregar observacion"><button type="button" class="btn">Agregar</button></div><input type="hidden" name="observaciones_via2" value="<?= h((string) $data['observaciones_via2']) ?>"></div></div></div></div>
    <div class="fieldset"><h2 style="font-size:1rem;margin-bottom:10px;">Evidencias</h2><div class="grid"><div class="c12 field"><label class="label">Evidencia biologica</label><textarea name="evidencia_biologica"><?= h((string) $data['evidencia_biologica']) ?></textarea></div><div class="c12 field"><label class="label">Evidencia fisica</label><textarea name="evidencia_fisica"><?= h((string) $data['evidencia_fisica']) ?></textarea></div><div class="c12 field"><label class="label">Evidencia material</label><textarea name="evidencia_material"><?= h((string) $data['evidencia_material']) ?></textarea></div></div></div>
    <div class="actions"><a class="btn ghost" href="<?= h($backUrl) ?>">Cancelar</a><a class="btn ghost" href="itp_eliminar.php?id=<?= (int) $id ?>">Eliminar</a><button class="btn primary" type="submit">Guardar</button></div>
  </form>
</div>
...
<script>
(function(){const gpsInput=document.getElementById('ubicacion_gps');const viewBtn=document.getElementById('btnViewMap');const markerBtn=document.getElementById('btnGps');const googleLink=document.getElementById('btnGoogleMaps');function gps(){return gpsInput?String(gpsInput.value||'').trim():'';}function syncLink(){const value=gps();if(!googleLink)return;if(value){googleLink.href='https://www.google.com/maps?q='+encodeURIComponent(value);googleLink.removeAttribute('aria-disabled');}else{googleLink.href='#';googleLink.setAttribute('aria-disabled','true');}}viewBtn?.addEventListener('click',function(){if(!gps()){alert('Primero registra una ubicacion GPS.');return;}markerBtn?.click();});googleLink?.addEventListener('click',function(ev){if(!gps()){ev.preventDefault();alert('Primero registra una ubicacion GPS.');}});gpsInput?.addEventListener('input',syncLink);gpsInput?.addEventListener('change',syncLink);syncLink();})();
</script>
