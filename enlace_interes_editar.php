...
if ($id <= 0) {
    ...
    exit('Falta id del enlace.');
}
$row = $service->enlaceInteresDetalle($id);
if ($row === null) {
...
$error = '';
$success = '';
$data = $service->enlaceInteresDefault($row);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $service->saveEnlaceInteres($data, $id);
        $success = 'Enlace actualizado correctamente.';
        $row = $service->enlaceInteresDetalle($id);
        $data = $service->enlaceInteresDefault($row);
    } catch (Throwable $e) {
        $error = $e->getMessage();
...
<head>
<meta charset="utf-8">
<title>Editar enlace de interes</title>
...
<div class="wrap">
  <div class="toolbar">
    <div>
      <h1 style="margin:0;">Editar enlace de interes</h1>
      <div style="color:var(--muted);">Registro #<?= (int) $id ?></div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a class="btn" href="enlaces_interes_listar.php">Enlaces</a>
      <a class="btn primary" href="<?= h((string) $data['url']) ?>" target="_blank" rel="noopener noreferrer">Abrir</a>
    </div>
  </div>
  ...
  <form method="post" class="card" id="frmEnlace">
    ...
    <div class="grid">
      ...
      <div class="c12 field"><label class="label">URL*</label><input type="text" name="url" value="<?= h((string) $data['url']) ?>" required></div>
      <div class="c12 field"><label class="label">Descripcion</label><textarea name="descripcion"><?= h((string) $data['descripcion']) ?></textarea></div>
    </div>
    <div class="actions"><a class="btn" href="enlaces_interes_listar.php">Volver</a><button class="btn primary" type="submit">Guardar cambios</button></div>
  </form>
</div>
</body>
</html>
