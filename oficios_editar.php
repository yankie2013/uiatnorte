...
<script>
const accSel = document.getElementById('accidente_id'); const entidadSel = document.getElementById('entidad_id'); const subSel = document.getElementById('subentidad_id'); const personaSel = document.getElementById('persona_id'); const personaTextInp = document.getElementById('persona_id_text'); const personaManualInp = document.getElementById('persona_destino_manual'); const tipoSel = document.getElementById('tipo'); const asuntoSel = document.getElementById('asunto_id'); const motivoTxt = document.getElementById('motivo'); const fechaInp = document.getElementById('fecha_emision'); const anioInp = document.getElementById('anio_oficio'); const numInp = document.getElementById('numero_oficio'); let lastModal = null; let personaItemsCache = <?= json_encode($personasActuales, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
...
function fillDatalist(listId, items){ const list=document.getElementById(listId); if(!list) return; list.innerHTML=''; items.forEach((item)=>{ const option=document.createElement('option'); option.value=String(item.nombre||'').trim(); option.dataset.id=String(item.id||''); list.appendChild(option); }); }
