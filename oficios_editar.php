...
$showDiligenciasInicial = str_contains($asuntoActualMatch, 'informacion') && str_contains($asuntoActualMatch, 'diligenc');
...
<div class="toolbar">
  ...
  <form method="post" class="card">
    ...
    <div class="grid">
      ...
      <div class="c6"><label>Entidad destino*</label><div class="field-row"><div class="combo-wrap combo-menu"><input type="hidden" name="entidad_id" id="entidad_id" value="<?= h((string) $data['entidad_id']) ?>"><input type="text" id="entidad_id_text" value="<?= h($entidadDestinoTexto) ?>" placeholder="Escribe para buscar la entidad" autocomplete="off" required><div id="entidad_id_options" class="combo-suggestions" role="listbox" aria-label="Sugerencias de entidad"></div><div class="combo-hint">Escribe el nombre o las siglas y selecciona una entidad de la lista.</div></div><button class="btn mini" type="button" onclick="openCreate('entidad')">+</button></div></div>
      ...
      <div class="c6"><label>Persona destino</label><div class="field-row"><div class="combo-wrap"><input type="hidden" name="persona_id" id="persona_id" value="<?= h((string) $data['persona_id']) ?>"><input type="hidden" name="persona_destino_manual" id="persona_destino_manual" value="<?= h((string) ($data['persona_destino_manual'] ?? '')) ?>"><input type="text" id="persona_id_text" list="persona_id_options" value="<?= h($personaDestinoTexto) ?>" placeholder="Selecciona o escribe manualmente"><datalist id="persona_id_options"><?php foreach ($personasActuales as $persona): ?><option value="<?= h(trim((string) $persona['nombre'])) ?>" data-id="<?= h((string) $persona['id']) ?>"></option><?php endforeach; ?></datalist><div class="combo-hint">Puedes elegir una persona registrada o escribirla manualmente. Si escribes aqui, solo se guardara en este oficio.</div></div><button class="btn mini" type="button" onclick="openCreate('persona')">+</button></div></div>
      ...
      <div class="c12" id="camaraRangoBox" style="display:none;"><div class="preview"><h4>Camara de video vigilancia</h4><div class="field-row" style="margin-bottom:10px; flex-wrap:wrap;"><div style="flex:1 1 220px;"><label for="camara_rango_desde">Entre las</label><input type="time" id="camara_rango_desde"></div><div style="flex:1 1 220px;"><label for="camara_rango_hasta">Hasta las</label><input type="time" id="camara_rango_hasta"></div></div><div class="muted">Al completar ambos campos se agregara al motivo una linea como: "Rango solicitado: entre las 08:00 hasta las 10:00".</div><div class="muted" style="margin-top:6px;">Marcadores disponibles en la plantilla Word: <strong>${oficio_rango_camaras}</strong>, <strong>${oficio_rango_desde}</strong> y <strong>${oficio_rango_hasta}</strong>.</div></div></div>
      <div class="c12"><label>Motivo / contexto*</label><textarea name="motivo" id="motivo" required><?= h($data['motivo']) ?></textarea></div>
      <div class="c12" id="diligenciasSolicitadasBox" style="<?= $showDiligenciasInicial ? 'display:block;' : 'display:none;' ?>"><div class="preview"><h4>Informacion de diligencias solicitada</h4><label for="diligencias_solicitadas">Diligencias*</label><textarea name="diligencias_solicitadas" id="diligencias_solicitadas" placeholder="Escribe una diligencia por linea" <?= $showDiligenciasInicial ? 'required' : '' ?>><?= h($data['diligencias_solicitadas']) ?></textarea><div class="muted">Puedes solicitar una o varias diligencias. Escribe cada una en una linea distinta.</div></div></div>
      <div class="c12"><label>Referencia</label><input type="text" name="referencia_texto" value="<?= h($data['referencia_texto']) ?>"></div>
      <div class="c6" id="vehiculoBox" style="<?= $showVehiculoInicial ? 'display:block;' : 'display:none;' ?>"><label>Vehículo involucrado</label><select name="involucrado_vehiculo_id" id="involucrado_vehiculo_id" <?= $showVehiculoInicial ? 'required' : '' ?>><option value="">Selecciona</option><?php foreach ($vehiculosActuales as $item): ?><option value="<?= h($item['id']) ?>" <?= (string) $data['involucrado_vehiculo_id'] === (string) $item['id'] ? 'selected' : '' ?>><?= h($item['nombre']) ?></option><?php endforeach; ?></select></div>
      <div class="c6" id="fallecidoBox" style="<?= ($showFallecidoInicial || $showInformeMedicoInicial) ? 'display:block;' : 'display:none;' ?>"><label id="personaInvolucradaLabel"><?= $showInformeMedicoInicial ? 'Persona herida, lesionada o fallecida' : 'Persona fallecida' ?></label><select name="involucrado_persona_id" id="involucrado_persona_id" <?= ($showFallecidoInicial || $showInformeMedicoInicial) ? 'required' : '' ?>><option value="">Selecciona</option><?php foreach ($personasCasoActuales as $item): ?><option value="<?= h($item['id']) ?>" <?= (string) $data['involucrado_persona_id'] === (string) $item['id'] ? 'selected' : '' ?>><?= h($item['nombre']) ?></option><?php endforeach; ?></select></div>
...
<script>
const accSel = document.getElementById('accidente_id'); const entidadSel = document.getElementById('entidad_id'); const entidadTextInp = document.getElementById('entidad_id_text'); const subSel = document.getElementById('subentidad_id'); const personaSel = document.getElementById('persona_id'); const personaTextInp = document.getElementById('persona_id_text'); const personaManualInp = document.getElementById('persona_destino_manual'); const tipoSel = document.getElementById('tipo'); const asuntoSel = document.getElementById('asunto_id'); const motivoTxt = document.getElementById('motivo'); const camaraRangoBox = document.getElementById('camaraRangoBox'); const camaraRangoDesdeInp = document.getElementById('camara_rango_desde'); const camaraRangoHastaInp = document.getElementById('camara_rango_hasta'); const fechaInp = document.getElementById('fecha_emision'); const anioInp = document.getElementById('anio_oficio'); const numInp = document.getElementById('numero_oficio'); const entidadOptionsBox = document.getElementById('entidad_id_options'); let lastModal = null; let entidadItemsCache = <?= json_encode($entidadesAutocomplete, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>; let personaItemsCache = <?= json_encode($personasActuales, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>; let lastEntidadLoaded = String(entidadSel ? (entidadSel.value || '') : ''); let entidadSuggestions = [];
...
function fillDatalist(listId, items, labelKey='nombre'){ const list=document.getElementById(listId); if(!list) return; list.innerHTML=''; items.forEach((item)=>{ const option=document.createElement('option'); option.value=String(item[labelKey]||'').trim(); option.dataset.id=String(item.id||''); list.appendChild(option); }); }
function closeEntidadSuggestions(){ if(entidadOptionsBox) entidadOptionsBox.classList.remove('open'); }
function openEntidadSuggestions(){ if(entidadOptionsBox && entidadOptionsBox.innerHTML.trim()!=='') entidadOptionsBox.classList.add('open'); }
function renderEntidadSuggestions(filterValue=''){ if(!entidadOptionsBox) return; const normalizedFilter=normalizeText(filterValue).trim(); entidadSuggestions=entidadItemsCache.filter((item)=>{ if(normalizedFilter==='') return true; return normalizeText(item.label||'').includes(normalizedFilter) || normalizeText(item.nombre||'').includes(normalizedFilter) || normalizeText(item.siglas||'').includes(normalizedFilter); }).slice(0,20); entidadOptionsBox.innerHTML=''; if(entidadSuggestions.length===0){ const empty=document.createElement('div'); empty.className='combo-empty'; empty.textContent='No hay coincidencias.'; entidadOptionsBox.appendChild(empty); openEntidadSuggestions(); return; } entidadSuggestions.forEach((item)=>{ const row=document.createElement('div'); row.className='combo-suggestion'; row.textContent=String(item.label||'').trim(); row.dataset.id=String(item.id||''); row.addEventListener('mousedown',(event)=>{ event.preventDefault(); selectEntidadSuggestion(item); }); entidadOptionsBox.appendChild(row); }); openEntidadSuggestions(); }
async function selectEntidadSuggestion(item){ if(!item || !entidadSel || !entidadTextInp) return; entidadSel.value=String(item.id||''); entidadTextInp.value=String(item.label||'').trim(); entidadTextInp.setCustomValidity(''); closeEntidadSuggestions(); await handleEntidadSelectionChange(); }
function setEntidadTextById(entidadId){ if(!entidadTextInp) return; const matched=entidadItemsCache.find((item)=>String(item.id||'')===String(entidadId||'')); entidadTextInp.value=matched ? String(matched.label||'').trim() : ''; }
function clearEntidadDependents(){ fillSelect(subSel, [], '', 'Ninguna'); personaItemsCache=[]; fillDatalist('persona_id_options', []); if(personaSel) personaSel.value=''; if(personaTextInp) personaTextInp.value=''; if(personaManualInp) personaManualInp.value=''; fillSelect(asuntoSel, [], '', 'Selecciona el asunto'); const asuntoPreview=document.getElementById('asuntoPreview'); const asuntoVarBox=document.getElementById('asuntoVarBox'); const asuntoVarSelect=document.getElementById('asuntoVarSelect'); if(asuntoPreview) asuntoPreview.style.display='none'; if(asuntoVarBox) asuntoVarBox.style.display='none'; if(asuntoVarSelect) asuntoVarSelect.innerHTML=''; }
function syncEntidadDestino(){ if(!entidadTextInp || !entidadSel) return { changed:false, matched:false, value:'' }; entidadTextInp.setCustomValidity(''); const typed=entidadTextInp.value.trim(); const currentValue=String(entidadSel.value||''); if(typed===''){ entidadSel.value=''; return { changed: currentValue !== '', matched:false, value:'' }; } const typedNormalized=normalizeText(typed); const matched=entidadItemsCache.find((item)=>{ const label=normalizeText(item.label||''); const nombre=normalizeText(item.nombre||''); const siglas=normalizeText(item.siglas||''); return typedNormalized===label || typedNormalized===nombre || (siglas!=='' && typedNormalized===siglas); }); if(!matched){ entidadSel.value=''; return { changed: currentValue !== '', matched:false, value:'' }; } entidadSel.value=String(matched.id||''); entidadTextInp.value=String(matched.label||'').trim(); return { changed: currentValue !== entidadSel.value, matched:true, value: entidadSel.value }; }
function syncPersonaDestinoManual(){ if(!personaTextInp || !personaSel || !personaManualInp) return; const typed=personaTextInp.value.trim(); if(typed===''){ personaSel.value=''; personaManualInp.value=''; return; } const matched=personaItemsCache.find((item)=>String(item.nombre||'').trim().toLowerCase()===typed.toLowerCase()); if(matched){ personaSel.value=String(matched.id||''); personaManualInp.value=''; personaTextInp.value=String(matched.nombre||'').trim(); return; } personaSel.value=''; personaManualInp.value=typed; }
function normalizeText(value){ return String(value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''); }
function stripCamaraRangeLine(text){ return String(text||'').split(/\r?\n/).filter((line)=>!normalizeText(line).startsWith('rango solicitado:')).join('\n').trim(); }
function extractCamaraRange(text){ const match=String(text||'').match(/Rango solicitado:\s*entre las\s*([0-2]\d:\d{2})\s*hasta las\s*([0-2]\d:\d{2})/i); return { desde: match ? match[1] : '', hasta: match ? match[2] : '' }; }
function camaraRangeLine(){ if(!camaraRangoDesdeInp || !camaraRangoHastaInp) return ''; const desde=String(camaraRangoDesdeInp.value||'').trim(); const hasta=String(camaraRangoHastaInp.value||'').trim(); if(!desde || !hasta) return ''; return 'Rango solicitado: entre las '+desde+' hasta las '+hasta+'.'; }
function syncCamaraRangeIntoMotivo(){ if(!motivoTxt) return; const base=stripCamaraRangeLine(motivoTxt.value); const line=camaraRangeLine(); motivoTxt.value=line ? (base ? (base+'\n'+line) : line) : base; }
function hydrateCamaraRangeFromMotivo(){ if(!camaraRangoDesdeInp || !camaraRangoHastaInp || !motivoTxt) return; const parsed=extractCamaraRange(motivoTxt.value); camaraRangoDesdeInp.value=parsed.desde; camaraRangoHastaInp.value=parsed.hasta; }
...
async function loadPersonas(entidadId, selected=''){ if(!entidadId){ personaItemsCache=[]; fillDatalist('persona_id_options', []); if(personaSel) personaSel.value=''; return; } const data = await fetchJSON('?ajax=personas&entidad_id='+encodeURIComponent(entidadId)); personaItemsCache=data.items||[]; fillDatalist('persona_id_options', personaItemsCache); if(selected){ const matched=personaItemsCache.find((item)=>String(item.id)===String(selected)); if(matched && personaTextInp){ personaTextInp.value=String(matched.nombre||'').trim(); } } syncPersonaDestinoManual(); }
async function loadAsuntos(entidadId, tipo, selected=''){ if(!entidadId){ fillSelect(asuntoSel, [], '', 'Selecciona el asunto'); return; } const data = await fetchJSON('?ajax=asuntos&entidad_id='+encodeURIComponent(entidadId)+'&tipo='+encodeURIComponent(tipo||'SOLICITAR')); fillSelect(asuntoSel, data.items||[], selected, 'Selecciona el asunto'); }
...
function asuntoEsNecropsia(){ const text=asuntoTexto(); const normalized=normalizeText(text); return text.includes('protocolo de necropsia') || text.includes('protocolo de autopsia') || text.includes('necropsia') || (normalized.includes('identificacion') && normalized.includes('cadaver')); }
function asuntoEsCamaraVideo(){ const text=normalizeText(asuntoTexto()); return text.includes('camara') && text.includes('video'); }
function asuntoEsSunarpHistorial(){ const text=normalizeText(asuntoTexto()); return text.includes('historial') && text.includes('transferenc'); }
function asuntoEsInformacionCertificado(){ const text=normalizeText(asuntoTexto()); return text.includes('informacion') && text.includes('certificado'); }
function asuntoEsIdentificacionVehiculo(){ const text=normalizeText(asuntoTexto()); return text.includes('identificacion') && text.includes('vehiculo'); }
function asuntoEsInformacionDiligencias(){ const text=normalizeText(asuntoTexto()); return text.includes('informacion') && text.includes('diligenc'); }
function asuntoEsInformeMedico(){ const text=normalizeText(asuntoTexto()); return text.includes('informe') && text.includes('medico'); }
...
async function loadPersonasInformeMedico(selected=''){ const sel=document.getElementById('involucrado_persona_id'); if(!accSel.value){ fillSelect(sel, [], '', 'Selecciona'); return; } const data=await fetchJSON('?ajax=personas_informe_medico&accidente_id='+encodeURIComponent(accSel.value)); fillSelect(sel,data.items||[],selected,'Selecciona'); }
async function toggleBoxesPorAsunto(){ const vehBox=document.getElementById('vehiculoBox'); const fallBox=document.getElementById('fallecidoBox'); const vehSel=document.getElementById('involucrado_vehiculo_id'); const personaSel=document.getElementById('involucrado_persona_id'); const personaLabel=document.getElementById('personaInvolucradaLabel'); const diligenciasBox=document.getElementById('diligenciasSolicitadasBox'); const diligenciasInput=document.getElementById('diligencias_solicitadas'); const requiresVehicle=asuntoEsPeritaje() || asuntoEsSunarpHistorial() || asuntoEsInformacionCertificado() || asuntoEsIdentificacionVehiculo(); if(requiresVehicle){ vehBox.style.display='block'; if(vehSel) vehSel.required=true; await loadVehiculosAccidente(vehSel ? vehSel.value : ''); } else { vehBox.style.display='none'; if(vehSel){ vehSel.required=false; vehSel.value=''; } } if(asuntoEsInformeMedico()){ fallBox.style.display='block'; personaSel.required=true; if(personaLabel) personaLabel.textContent='Persona herida, lesionada o fallecida'; await loadPersonasInformeMedico(personaSel.value); } else if(asuntoEsNecropsia()){ fallBox.style.display='block'; personaSel.required=true; if(personaLabel) personaLabel.textContent='Persona fallecida'; await loadFallecidosAccidente(personaSel.value); } else { fallBox.style.display='none'; personaSel.required=false; personaSel.value=''; } if(camaraRangoBox){ const isCamara=asuntoEsCamaraVideo(); camaraRangoBox.style.display=isCamara?'block':'none'; if(isCamara){ hydrateCamaraRangeFromMotivo(); syncCamaraRangeIntoMotivo(); } else if(motivoTxt){ motivoTxt.value=stripCamaraRangeLine(motivoTxt.value); } } if(diligenciasBox&&diligenciasInput){ const active=asuntoEsInformacionDiligencias(); diligenciasBox.style.display=active?'block':'none'; diligenciasInput.required=active; } }
