...
async function toggleBoxesPorAsunto(){ const vehBox=document.getElementById('vehiculoBox'); const fallBox=document.getElementById('fallecidoBox'); const vehSel=document.getElementById('involucrado_vehiculo_id'); const requiresVehicle=asuntoEsPeritaje() || asuntoEsSunarpHistorial() || asuntoEsInformacionCertificado(); if(requiresVehicle){ vehBox.style.display='block'; if(vehSel) vehSel.required=true; await loadVehiculosAccidente(vehSel ? vehSel.value : ''); } else { vehBox.style.display='none'; if(vehSel){ vehSel.required=false; vehSel.value=''; } } if(asuntoEsNecropsia()){ fallBox.style.display='block'; await loadFallecidosAccidente(document.getElementById('involucrado_persona_id').value); } else { fallBox.style.display='none'; document.getElementById('involucrado_persona_id').value=''; } if(camaraRangoBox){ const isCamara=asuntoEsCamaraVideo(); camaraRangoBox.style.display=isCamara?'block':'none'; if(isCamara){ hydrateCamaraRangeFromMotivo(); syncCamaraRangeIntoMotivo(); } else if(motivoTxt){ motivoTxt.value=stripCamaraRangeLine(motivoTxt.value); } } }
