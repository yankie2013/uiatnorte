...
<div class="toolbar">
  ...
  <form method="post" class="card">
    ...
    <div class="grid">
      ...
      <div class="c6" id="fallecidoBox" style="<?= $showFallecidoInicial ? 'display:block;' : 'display:none;' ?>"><label>Persona fallecida</label><select name="involucrado_persona_id" id="involucrado_persona_id" <?= $showFallecidoInicial ? 'required' : '' ?>><option value="">Selecciona</option><?php foreach ($fallecidosActuales as $item): ?><option value="<?= h($item['id']) ?>" <?= (string) $data['involucrado_persona_id'] === (string) $item['id'] ? 'selected' : '' ?>><?= h($item['nombre']) ?></option><?php endforeach; ?></select></div>
...
async function toggleBoxesPorAsunto(){ const vehBox=document.getElementById('vehiculoBox'); const fallBox=document.getElementById('fallecidoBox'); const vehSel=document.getElementById('involucrado_vehiculo_id'); const diligenciasBox=document.getElementById('diligenciasSolicitadasBox'); const diligenciasInput=document.getElementById('diligencias_solicitadas'); const requiresVehicle=asuntoEsPeritaje() || asuntoEsSunarpHistorial() || asuntoEsInformacionCertificado() || asuntoEsIdentificacionVehiculo(); if(requiresVehicle){ vehBox.style.display='block'; if(vehSel) vehSel.required=true; await loadVehiculosAccidente(vehSel ? vehSel.value : ''); } else { vehBox.style.display='none'; if(vehSel){ vehSel.required=false; vehSel.value=''; } } if(asuntoEsNecropsia()){ fallBox.style.display='block'; await loadFallecidosAccidente(document.getElementById('involucrado_persona_id').value); } else { fallBox.style.display='none'; document.getElementById('involucrado_persona_id').value=''; } if(camaraRangoBox){ const isCamara=asuntoEsCamaraVideo(); camaraRangoBox.style.display=isCamara?'block':'none'; if(isCamara){ hydrateCamaraRangeFromMotivo(); syncCamaraRangeIntoMotivo(); } else if(motivoTxt){ motivoTxt.value=stripCamaraRangeLine(motivoTxt.value); } } if(diligenciasBox&&diligenciasInput){ const active=asuntoEsInformacionDiligencias(); diligenciasBox.style.display=active?'block':'none'; diligenciasInput.required=active; } }
