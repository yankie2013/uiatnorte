...
if ($returnTo === '') {
    $returnTo = 'involucrados_vehiculos_listar.php?accidente_id=' . (int)$row['accidente_id'];
}
...
if (!empty($row['anio'])) {
    $vehiculo .= ($vehiculo !== '' ? ' ' : '') . '(' . $row['anio'] . ')';
}

$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['confirm'] ?? '') === '1') {
    try {
        $delete = $pdo->prepare("DELETE FROM involucrados_vehiculos WHERE id = ? LIMIT 1");
        $delete->execute([$id]);
...
<title>Eliminar Involucrado de Vehiculo</title>
<style>
...
<div class="wrap">
  ...
  <div class="meta">
    ...
    <div class="row"><div class="lbl">Tipo</div><div class="val"><?= h($row['tipo'] ?? '-') ?></div></div>
