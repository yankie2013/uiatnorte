...
{
    $table = $section->addTable('PairTableDatosGenerales');
...
function save_template_doc_dg(string $templatePath, string $tmpPath, array $markers): void
{
...
$filename = uiat_docx_filename(['DATOS_GENERALES', 'Accidente', $accidenteId], 'DATOS_GENERALES_' . $accidenteId);
$templatePath = __DIR__ . '/plantillas/word_informe_datos_generales.docx';
...
if ($useTemplate) {
    ...
    $tmp = tempnam($tmpDir, 'dgtpl_');
    if ($tmp === false) {
        http_response_code(500);
        exit('No se pudo crear el archivo temporal.');
    }

    save_template_doc_dg($templatePath, $tmp, $markers);
...
$phpWord->addTableStyle('PairTableDatosGenerales', [
    'borderSize' => 6,
    'borderColor' => 'D1D5DB',
    'cellMargin' => 80,
]);
...
$section->addText('INFORME DE DATOS GENERALES DEL ACCIDENTE', ['bold' => true, 'size' => 15], ['align' => 'center', 'spaceAfter' => 120]);
