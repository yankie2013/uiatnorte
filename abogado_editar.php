...
$detailUrl = str_starts_with($return, 'abogado_ver.php')
    ? $return
    : ('abogado_ver.php?id=' . $id . '&return=' . urlencode($return));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        header('Location: ' . $return . (str_contains($return, '?') ? '&' : '?') . 'ok=updated');
        exit;
    } catch (Throwable $e) {
        $error = $e->getMessage();
    }
...
<div class="wrap">
  <div class="toolbar">
    <div>
      <h1 style="margin:0 0 6px">Abogado <span class="badge">Editar</span></h1>
      <div class="small">Registro #<?= (int) $id ?> - Accidente ID: <?= (int) $accidenteId ?></div>
    </div>
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <a class="btn" href="<?= h($return) ?>">Volver</a>
      <a class="btn" href="<?= h($detailUrl) ?>">Ver detalle</a>
      <button class="btn primary" type="submit" form="frmAbogado">Guardar cambios</button>
    </div>
  </div>
  ...
  <div class="meta">
    <div class="meta-box">
      <div class="label">Persona representada</div>
      <div style="font-weight:700;margin-top:4px;"><?= h((string) ($row['persona_rep_nom'] ?? 'Sin persona')) ?></div>
    </div>
    <div class="meta-box">
      <div class="label">Condicion representada</div>
      <div style="font-weight:700;margin-top:4px;"><?= h((string) (($row['condicion_representado'] ?? '') !== '' ? $row['condicion_representado'] : 'Sin condicion')) ?></div>
    </div>
    <div class="meta-box">
      <div class="label">Ultima actualizacion</div>
      <div style="font-weight:700;margin-top:4px;"><?= h((string) (($row['actualizado_en'] ?? '') !== '' ? $row['actualizado_en'] : 'Sin fecha')) ?></div>
    </div>
  </div>
