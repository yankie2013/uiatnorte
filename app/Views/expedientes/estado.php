...
(function (PDO $pdo, int $id): void {
    $users=$pdo->query("SELECT id,nombre,grado,rol FROM usuarios WHERE activo=1 AND rol IN ('jefe_emi','adjunto') ORDER BY nombre")->fetchAll();
    $responsibleLabel = static fn($name, $grade = null) => trim(trim((string)$grade).' '.trim((string)$name)) ?: 'Sin asignar';
    $s=$pdo->prepare('SELECT a.*,u.nombre responsable,u.grado responsable_grado FROM accidentes a LEFT JOIN usuarios u ON u.id=a.responsable_id WHERE a.id=?');$s->execute([$id]);$a=$s->fetch();
    ...
    echo '<section><h2>Expediente #'.$id.' · '.Page::escape($a['registro_sidpol']??'Sin SIDPOL').'</h2><p>'.Page::escape($a['lugar']).'</p><p>Responsable: <strong>'.Page::escape($responsibleLabel($a['responsable'], $a['responsable_grado'])).'</strong></p><div class="actions"><a href="accidente_vista_tabs.php?accidente_id='.$id.'">Abrir expediente</a><a href="gestion_expedientes.php">Buscador general</a></div>';
    ...
    echo '</section>';
    $s=$pdo->prepare('SELECT c.*,u.nombre,u.grado FROM expediente_colaboradores c JOIN usuarios u ON u.id=c.usuario_id WHERE c.accidente_id=? AND c.revocado_en IS NULL');$s->execute([$id]);$collabs=$s->fetchAll();
    echo '<section><h2>Adjuntos con colaboración activa</h2><p>Al revocar la colaboración se conserva el historial y el acceso de consulta.</p>';
    ...
    echo '</section><section><h2>Transferencias</h2><p>El responsable cambia cuando el destinatario acepta. Al aceptar, se cierran las colaboraciones anteriores.</p>';
    $s=$pdo->prepare('SELECT t.*,u.nombre destino,u.grado destino_grado FROM expediente_transferencias t JOIN usuarios u ON u.id=t.destino_id WHERE t.accidente_id=? ORDER BY t.id DESC');$s->execute([$id]);$transfers=$s->fetchAll();$pending=false;
