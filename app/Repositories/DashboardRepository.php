...
final class DashboardRepository
{
    ...
    public function years(): array
    {
        return array_map('intval', $this->pdo->query(
            'SELECT DISTINCT YEAR(fecha_accidente) FROM accidentes WHERE fecha_accidente >= \'1000-01-01\' ORDER BY 1 DESC'
        )->fetchAll(PDO::FETCH_COLUMN));
    }

    public function snapshot(?int $year): array
    {
        $where = $year === null ? '' : ' WHERE a.fecha_accidente >= ? AND a.fecha_accidente < ?';
        ...
        $states = $read("SELECT COALESCE(NULLIF(TRIM(a.estado), ''), 'Pendiente') AS label, COUNT(*) AS total
                        FROM accidentes a $where GROUP BY label");
        ...
        $summary = $read("SELECT COUNT(*) AS total, SUM(a.priority = 1 AND COALESCE(NULLIF(TRIM(a.estado), ''), 'Pendiente') IN ('Pendiente', 'Con diligencias')) AS priority,
                         ...
                         FROM accidentes a $where")[0];
        $bucket = $year === null ? 'YEAR(a.fecha_accidente)' : 'MONTH(a.fecha_accidente)';
        $dateGuard = $where === '' ? " WHERE a.fecha_accidente >= '1000-01-01'" : $where;
        $timeline = $read("SELECT $bucket AS period, COUNT(*) AS total FROM accidentes a $dateGuard GROUP BY period ORDER BY period");
        ...
        $recent = $read("SELECT a.id, a.registro_sidpol, a.fecha_accidente, a.creado_en,
                        ...
                        FROM accidentes a LEFT JOIN comisarias c ON c.id = a.comisaria_id $where
