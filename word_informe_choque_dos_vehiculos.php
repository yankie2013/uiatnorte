...
$MODS=''; 
try{
  $q=$pdo->prepare("SELECT m.nombre FROM accidente_modalidad_activos am JOIN modalidad_accidente m ON m.id=am.modalidad_id WHERE am.accidente_id=:a ORDER BY m.nombre");
...
$CONS='';
try{
  $q=$pdo->prepare("SELECT c.nombre FROM accidente_consecuencia_activos ac JOIN consecuencia_accidente c ON c.id=ac.consecuencia_id WHERE ac.accidente_id=:a");
...
function get_documento_vehiculo(PDO $pdo, $vehiculo_id, $accidente_id, $iv_id=null){
  // 1) por vehiculo_id
  if ($vehiculo_id) {
    $q=$pdo->prepare("SELECT * FROM documento_vehiculo_activos WHERE vehiculo_id=:v ORDER BY id DESC LIMIT 1");
  ...
  if ($iv_id) {
    $q=$pdo->prepare("SELECT * FROM documento_vehiculo_activos WHERE involucrado_vehiculo_id=:iv ORDER BY id DESC LIMIT 1");
...
function get_abogado(PDO $pdo, $accidente_id, $persona_id){
  ...
  $r=$q->fetch(PDO::FETCH_ASSOC);
  if($r) return $r;
  $q=$pdo->prepare("SELECT * FROM abogados_activos WHERE persona_id=:p ORDER BY id DESC LIMIT 1");
  $q->execute([':p'=>$persona_id]); return $q->fetch(PDO::FETCH_ASSOC) ?: [];
}
...
function get_familiar_fallecido(PDO $pdo, $accidente_id, $occ_persona_id=null, $occ_inv_id=null){
  ...
  if(!$inv_id && $occ_persona_id){
    $q=$pdo->prepare("SELECT id FROM involucrados_personas_activos WHERE accidente_id=:a AND persona_id=:p ORDER BY id DESC LIMIT 1");
...
function get_efectivos_intervinientes(PDO $pdo, $accidente_id){
  $sql="
    SELECT ep.*,
      ...
      FROM policial_interviniente_activos ep
